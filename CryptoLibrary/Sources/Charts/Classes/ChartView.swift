...
public final class ChartView: UIView {
    ...
    public func enableChannels(at indexes: [Int]) {
    ...
    public func disableChannels(at indexes: [Int]) {
        indexes.forEach { index in
    ...
    private func addDetailsView(to location: CGPoint) {
        ...
        let detailsPointerView = UIView()
        detailsPointerView.backgroundPalette = Palette.chartLine
        chartRenderer.addSubview(detailsPointerView)
        
        self.detailsPointerView = detailsPointerView

        let detailsView = ChartDetailsView(
    ...
    private func moveDetailsView(to location: CGPoint, animated: Bool) {
        ...
        var originX = locationX - detailsViewOffset - (detailsView?.frame.width ?? .zero)
        if originX < detailsViewOffset {
            originX += (detailsView?.frame.width ?? .zero) + detailsViewOffset * 2
        }
        ...
        case .point:
            ...
            fallthrough
        case .none:
            detailsPointerView?.frame = CGRect(
